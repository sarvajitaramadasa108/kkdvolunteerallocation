...
function RegistrationSection({ title, description, rows, services, drafts, updateDraft, saveRow, saving, assigned, loading, filterValues = {}, setFilterValues }) {
...
function filterRows(rows, query, columnFilters = {}) {
  ...
  return rows.filter((row) => {
    ...
    const matchesColumns = Object.entries(columnFilters).every(([column, value]) => !String(value || "").trim() || String(row[column] || "").toLowerCase().includes(String(value).trim().toLowerCase()));
