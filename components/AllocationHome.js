...
function ServiceWiseSection({ services, rows, selectedService, setSelectedService, loading, onRefresh }) {
  function downloadExcel() {
    ...
    const headers = ["Full Name", "Age", "Mobile Number", "Gender", "Availability on 2nd", "Availability on 3rd", "Availability on 4th", "Availability on 5th", "Availability on 6th", "Service", "Category"];
    const escape = (value) => String(value ?? "").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;");
    const cells = (row) => [row.fullName, row.age, row.mobile, row.gender, row.availabilityOn2, row.availabilityOn3, row.availabilityOn4, row.availabilityOn5, row.availabilityOn6, row.serviceName, row.category];
