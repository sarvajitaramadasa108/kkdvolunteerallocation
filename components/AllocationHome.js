...
function ServiceWiseSection({ services, rows, selectedService, setSelectedService, loading, onRefresh }) {
  function downloadExcel() {
    ...
    const headers = ["Full Name", "Age", "Mobile Number", "Gender", "College Name", "Availability on 3rd", "Availability on 4th", "Availability on 5th", "Service", "Category"];
    const escape = (value) => String(value ?? "").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;");
    const cells = (row) => [row.fullName, row.age, row.mobile, row.gender, row.collegeName, row.availabilityOn3, row.availabilityOn4, row.availabilityOn5, row.serviceName, row.category];
...
function filterRows(rows, query, columnFilters = {}) {
  ...
  return rows.filter((row) => {
    const matchesQuery = !needle || [row.fullName, row.age, row.mobile, row.gender, row.collegeName, row.serviceName, row.category].some((value) => String(value || "").toLowerCase().includes(needle));
