...
export default function AllocationHome({ view = "home" }) {
  ...
  const hasLiveFilters = Boolean(String(query || "").trim() || Object.values(liveColumnFilters).some((value) => String(value || "").trim()));
...
function ServiceWiseSection({ services, rows, selectedService, setSelectedService, loading, onRefresh }) {
  function downloadExcel() {
    ...
    const headers = ["Name", "Age", "WhatsApp Number", "Gender", "No. of Round Chanting", "Address", "Service", "Category", "Photo"];
    const escape = (value) => String(value ?? "").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;");
    const cells = (row) => [row.fullName, row.age, row.mobile, row.gender, row.roundsChanting, row.address, row.serviceName, row.category, row.photo];
  ...
  return <><section className="service-toolbar"><div><label htmlFor="service-filter">Select service</label><select id="service-filter" value={selectedService} onChange={(event) => setSelectedService(event.target.value)}><option value="">Choose a service</option>{services.map((service) => <option key={service.serviceName} value={service.serviceName}>{service.serviceName}</option>)}</select></div><div className="service-toolbar-actions"><button className="refresh-button" onClick={onRefresh}>{loading ? "Loading..." : "Refresh"}</button><button className="download-button" onClick={downloadExcel} disabled={!selectedService || !rows.length}>Download Excel</button></div></section><section className="registration-section"><div className="section-title"><div><h2>{selectedService || "Service wise volunteers"}</h2><p>{selectedService ? `${rows.length} assigned volunteers` : "Choose a service to view assigned volunteers."}</p></div></div>{loading ? <div className="loading">Loading assigned volunteers...</div> : selectedService ? <RegistrationSection title="" description="Assigned rows are read from Volunteer Master and Assignment Map." rows={rows} services={services} drafts={{}} updateDraft={() => {}} saveRow={() => {}} saving="" assigned hideReferenceServices loading={false} /> : <div className="empty">Select a service above.</div>}</section></>;
}

function RegistrationSection({ title, description, rows, totalRows = rows.length, services, drafts, updateDraft, saveRow, saving, assigned, hideReferenceServices = false, loading, filterValues = {}, setFilterValues, filterOptions = {} }) {
  const filterable = Boolean(setFilterValues);
  const compact = hideReferenceServices || assigned;
  const columns = ["fullName", "age", "mobile", "gender", "roundsChanting", ...(compact ? [] : ["lastYearService"]), "address", ...(compact ? [] : ["thisYearService"]), "serviceName", "category", "photo"];
  const headers = ["Name", "Age", "WhatsApp Number", "Gender", "No. of Round Chanting", ...(compact ? [] : ["Last Year Service"]), "Address", ...(compact ? [] : ["This Year Service"]), "Service", "Category", "Photo", "Action"];
  return <section className="registration-section"><div className="section-title"><div><h2>{title}</h2><p>{description}</p></div><span className="row-count">{rows.length === totalRows ? `${rows.length} rows` : `Showing ${rows.length} of ${totalRows} rows`}</span></div>{loading ? <div className="loading">Loading {title.toLowerCase()}...</div> : !totalRows ? <div className="empty">No records found.</div> : <div className="table-frame"><table><thead><tr>{headers.map((header) => <th key={header}>{header}</th>)}</tr>{filterable ? <tr className="filter-row">{columns.map((column) => <th key={column}>{filterOptions[column] ? <select value={filterValues[column] || ""} onChange={(event) => setFilterValues((current) => ({ ...current, [column]: event.target.value }))} aria-label={`Filter ${column}`}><option value="">All</option>{filterOptions[column].map((option) => <option key={option} value={option}>{option}</option>)}</select> : <input value={filterValues[column] || ""} onChange={(event) => setFilterValues((current) => ({ ...current, [column]: event.target.value }))} placeholder="Filter" aria-label={`Filter ${column}`} />}</th>)}<th /></tr> : null}</thead><tbody>{rows.length ? rows.map((row) => <RegistrationRow key={row.responseKey} row={row} services={services} drafts={drafts} updateDraft={updateDraft} saveRow={saveRow} saving={saving} assigned={assigned} hideReferenceServices={hideReferenceServices} />) : <tr><td className="no-match" colSpan={headers.length}>No matching records. Adjust or clear the filters.</td></tr>}</tbody></table></div>}</section>;
}

function RegistrationRow({ row, services, drafts, updateDraft, saveRow, saving, assigned, hideReferenceServices = false }) {
  const draft = drafts[row.responseKey] || { serviceName: row.serviceName || "", category: row.category || "" };
  const compact = hideReferenceServices || assigned;
  return <tr><td>{row.fullName || "-"}</td><td>{row.age || "-"}</td><td>{row.mobile || "-"}</td><td>{row.gender || "-"}</td><td>{row.roundsChanting || "-"}</td>{compact ? null : <td>{row.lastYearService || "-"}</td>}<td>{row.address || "-"}</td>{compact ? null : <td>{row.thisYearService || "-"}</td>}<td><select value={draft.serviceName} onChange={(event) => updateDraft(row, "serviceName", event.target.value)} disabled={saving === row.responseKey}><option value="">Select service</option>{services.map((service) => <option key={service.serviceName} value={service.serviceName}>{service.serviceName}</option>)}</select></td><td><select className="category-select" value={draft.category} onChange={(event) => updateDraft(row, "category", event.target.value)} disabled={saving === row.responseKey}><option value="">Select category</option>{CATEGORIES.map((category) => <option key={category} value={category}>{category}</option>)}</select></td><td>{row.photo ? <a href={row.photo} target="_blank" rel="noreferrer">View photo</a> : "-"}</td><td><button className="save-button" onClick={() => saveRow(row)} disabled={saving === row.responseKey}>{saving === row.responseKey ? "Saving..." : assigned ? "Save changes" : "Assign"}</button></td></tr>;
}
