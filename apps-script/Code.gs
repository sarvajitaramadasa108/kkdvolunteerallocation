const SPREADSHEET_ID = "1shcj7RbQRuMAVq5phvkrx9NCpfJkPA7LJq0wPbhCEqM";
const FORM_SHEET = "Volunteer Master";
...
function mapRegistration_(row, sourceRow, map) {
  ...
  return { responseKey: (timestamp || "row") + "::" + sourceRow, sourceRow: sourceRow, fullName: cell_(row, map.fullName), age: cell_(row, map.age), mobile: cell_(row, map.mobile), gender: cell_(row, map.gender), roundsChanting: cell_(row, map.roundsChanting), lastYearService: cell_(row, map.lastYearService), address: cell_(row, map.address), photo: cell_(row, map.photo), thisYearService: cell_(row, map.thisYearService), availabilityOn3: availability(map.day3), availabilityOn4: availability(map.day4), availabilityOn5: availability(map.day5) };
}
